...
uint16_t cvfilter = 0;  // filter freq
uint16_t res = 65535;   // filter Q
uint16_t sel = 0;       // filter type
...
void loop() {
  ...
  if (ReadReg(SCI_AICTRL0) > 32){
    ...
    for (int i = 0; i < 32; i++) {
      ...
      int16_t sample = Svf_process(svf, Saw_process(saw, cv), cvfilter, res, sel) >> 2;
