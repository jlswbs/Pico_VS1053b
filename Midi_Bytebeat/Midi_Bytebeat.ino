...
void pitch(byte chan, int pitch){

  char low = pitch & 0x7F;
  char high = (pitch >> 7) & 0x7F;  
...
void loop(){
  ...
  uint16_t notepitch = map(analogRead(A2), MINADC, MAXADC, 0, 16383);

  uint16_t del = map(analogRead(A3), MINADC, MAXADC, 239, 99);

  switch(algos){

    case  0: out = t*(t>>9|t>>10)%(t|t>>9); break;
    case  1: out = t*(t>>9&t>>13)%(t|t>>9); break;
    case  2: out = t&(t>>9|t>>13)%(t|t>>9); break;
    case  3: out = t|(t>>6^t>>13)%(t|t>>9); break;
    case  4: out = t^(t>>9^t>>13)+(t|t>>9); break;
    case  5: out = t^(t>>9^t>>13)-(t|t>>9); break;
    case  6: out = (t>>9^t>>13)-(t|t>>9); break;
    case  7: out = (t<<2^t>>10)-(t|t>>9); break;
    case  8: out = (t<<0^t>>10)-(t|t>>9); break;
    case  9: out = ((t<<t)^t>>10)+(t|t>>9); break;
    case 10: out = ((t<<t)^t>>10)+(t^t>>9); break;
  ...
  if (cnt == del) {
    ...
    note_on(0, out%127, 64, 63 + out%64);
    note_on(1, 127 - out%127, 56, 63 + out%64);
    
    if(dr) {
      
      note_on(9, 0, 35, 64);
      note_on(9, 0, 36 + out%36, 24 + rand()%48);
