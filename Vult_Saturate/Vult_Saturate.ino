...
Saw_process_type saw1;
Saw_process_type saw2;
...
void setup() {
  ...
  Saw_process_init(saw1);
  Saw_process_init(saw2);  
...
void loop() {
  ...
  if (ReadReg(SCI_AICTRL0) > 32){
    ...
    for (int i = 0; i < 32; i++) {
      ...
      int16_t sample = Saturate_process(Saw_process(saw1, cv) + Saw_process(saw2, cv+100)) >> 2;     
