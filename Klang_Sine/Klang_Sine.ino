...
void setup() {
  ...
  Klang::connect(mOsc, Node::CH_OUT_SIGNAL, mDAC, Node::CH_IN_SIGNAL);

  mOsc.set_frequency(440);
  mOsc.set_amplitude(0.75);
...
void loop() {
  ...
  if (ReadReg(SCI_AICTRL0) > 32){
    ...
    for (int i = 0; i < 32; i++) {
      ...
      mDAC.process_frame(buffer, buffer);
      int16_t sample = buffer[0] * (1 << 14);
