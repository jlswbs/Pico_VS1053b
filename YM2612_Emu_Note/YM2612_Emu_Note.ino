// YM2612 FM sound chip emulator - random note //
...
#define PAL_CLK           7600489
#define NTSC_CLK          7670453
#define PAL_FRQ           52781
#define NTSC_FRQ          53267
...
void setup(){
  ...
  YM2612_Init(PAL_CLK, PAL_FRQ, 0);
  YM2612_Reset();

  buffer = (int **)malloc(sizeof(int *) * 2);
  buffer[0] = (int *)malloc(sizeof(int));
  buffer[1] = (int *)malloc(sizeof(int));

  YM2612_Write(0,0x22); //LFO off
  YM2612_Write(1,0); //LFO off

  YM2612_Write(0,0x27); //Channel 3 mode normal
  YM2612_Write(1,0); //Channel 3 mode normal

  YM2612_Write(0,0x28); //All channels off
  YM2612_Write(1,0); //All channels off
  
  YM2612_Write(0,0x28); //All channels off
  YM2612_Write(1,1); //All channels off  
 
  YM2612_Write(0,0x28); //All channels off
  YM2612_Write(1,2); //All channels off  

  YM2612_Write(0,0x28); //All channels off
  YM2612_Write(1,3); //All channels off  

  YM2612_Write(0,0x28); //All channels off
  YM2612_Write(1,4); //All channels off  

  YM2612_Write(0,0x28); //All channels off
  YM2612_Write(1,5); //All channels off    
  
  YM2612_Write(0,0x2B); //DAC off
  YM2612_Write(1,0); //DAC off    

  YM2612_Write(0,0x30); //DT1/MUL (multiplier) OP1
  YM2612_Write(1,0x71); //DT1/MUL (multiplier) OP1

  YM2612_Write(0,0x34); //DT1/MUL (multiplier) OP2
  YM2612_Write(1,0x0D); //DT1/MUL (multiplier) OP2
  
  YM2612_Write(0,0x38); //DT1/MUL (multiplier) OP3
  YM2612_Write(1,0x33); //DT1/MUL (multiplier) OP3

  YM2612_Write(0,0x3C); //DT1/MUL (multiplier) OP4
  YM2612_Write(1,0x01); //DT1/MUL (multiplier) OP4

  YM2612_Write(0,0x40); //Total Level OP1
  YM2612_Write(1,0x23); //Total Level OP1  

  YM2612_Write(0,0x44); //Total Level OP2
  YM2612_Write(1,0x2D); //Total Level OP2
  
  YM2612_Write(0,0x48); //Total Level OP3
  YM2612_Write(1,0x26); //Total Level OP3
  
  YM2612_Write(0,0x4C); //Total Level OP4
  YM2612_Write(1,0x00); //Total Level OP4

  YM2612_Write(0,0x50); //RS/AR OP1
  YM2612_Write(1,0x5F); //RS/AR OP1
  
  YM2612_Write(0,0x54); //RS/AR OP2
  YM2612_Write(1,0x99); //RS/AR OP2
  
  YM2612_Write(0,0x58); //RS/AR OP3
  YM2612_Write(1,0x5F); //RS/AR OP3
  
  YM2612_Write(0,0x5C); //RS/AR OP4
  YM2612_Write(1,0x94); //RS/AR OP4

  YM2612_Write(0,0x60); //RS/AM D1R
  YM2612_Write(1,5); //RS/AM D1R
  
  YM2612_Write(0,0x64); //RS/AM D1R
  YM2612_Write(1,5); //RS/AM D1R
  
  YM2612_Write(0,0x68); //RS/AM D1R
  YM2612_Write(1,5); //RS/AM D1R
  
  YM2612_Write(0,0x6C); //RS/AM D1R
  YM2612_Write(1,7); //RS/AM D1R
  
  YM2612_Write(0,0x70); //RS/ D2R
  YM2612_Write(1,2); //RS/ D2R

  YM2612_Write(0,0x74); //RS/D2R
  YM2612_Write(1,2); //RS/D2R
  
  YM2612_Write(0,0x78); //RS/D2R
  YM2612_Write(1,2); //RS/D2R

  YM2612_Write(0,0x7C); //RS/D2R
  YM2612_Write(1,2); //RS/D2R

  YM2612_Write(0,0x80); //D1L
  YM2612_Write(1,0x11); //D1L
  
  YM2612_Write(0,0x84); //D1L
  YM2612_Write(1,0x11); //D1L
  
  YM2612_Write(0,0x88); //D1L
  YM2612_Write(1,0x11); //D1L

  YM2612_Write(0,0x8C); //D1L
  YM2612_Write(1,0xA6); //D1L

  YM2612_Write(0,0x90); //Proprietary/?????
  YM2612_Write(1,0); //Proprietary/?????
  
  YM2612_Write(0,0x94); //Proprietary/?????
  YM2612_Write(1,0); //Proprietary/?????
  
  YM2612_Write(0,0x98); //Proprietary/?????
  YM2612_Write(1,0); //Proprietary/?????
  
  YM2612_Write(0,0x9C); //Proprietary/?????
  YM2612_Write(1,0); //Proprietary/?????

  YM2612_Write(0,0xb0); //feedback/algo
  YM2612_Write(1,0x32); //feedback/algo
  
  YM2612_Write(0,0xb4); //both speakers on
  YM2612_Write(1,0xC0); //both speakers on

  // MAGIC TIME!
  YM2612_Write(0,0x28); //key off
  YM2612_Write(1,0); //key off  

  YM2612_Write(0,0xA4); //set frequency (part 1 = octave)
  YM2612_Write(1,0x22); //set frequency (part 1 = octave)
  YM2612_Write(0,0xA0); //set frequency (part 2 = note)
  YM2612_Write(1,0x69); //set frequency (part 2 = note)  
...
void loop(){
  ...
  if (ReadReg(SCI_AICTRL0) > 32){  
    ...
    for (int i = 0; i < 32; i++) {
      ...
      YM2612_Update((int **)buffer, 32);
      YM2612_DacAndTimers_Update((int **)buffer, 32);
      int16_t sample = (buffer[0][0]+buffer[1][0]) >> 2;
...
void loop1(){
  ...
  YM2612_Write(0,0xA0);           //set frequency (part 2 = note)
  YM2612_Write(1,0x69-rand()%48); //set frequency (part 2 = note)    
  
  YM2612_Write(0,0x28); //key on
  YM2612_Write(1,0xC0); //key on
