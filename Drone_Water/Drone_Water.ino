...
void loop(){
  ...
  for (int y = 1; y < valh-1; y++) {
    ...
    for (int x = 1; x < valw-1; x++) {
      ...
      v[x][y] += (p[x-1][y] + p[x+1][y] + p[x][y-1] + p[x][y+1]) * a - p[x][y];
  ...
  for (int y = 0; y < valh; y++) {
    ...
    for (int x = 0; x < valw; x++) {
      ...
      uint8_t l = 255.0f * p[x][y];
      uint8_t r = 255.0f * v[x][y];
...
void loop1(){
  ...
  valw = map(analogRead(A1), MINADC, MAXADC, 8, WIDTH);
  valh = map(analogRead(A2), MINADC, MAXADC, 8, HEIGHT);
  a = map(analogRead(A3), MINADC, MAXADC, 1499, 2499);
  a /= 10000.0f;
