// VLSI 1053 codec PCM mixer - IIR sine filter drums + echoverb //
...
void loop(){
  ...
  if (ReadReg(SCI_AICTRL0) > 32){   
    ...
    for (int i = 0; i < 32; i++) { 
      ...
      int16_t sample = 32767.0f * echo_verb(drum.calculate(), mix);
...
void loop1(){
  ...
  drum.lim = lim;
  drum.decay = randomf(0.5f, 1.0f) / SAMPLE_RATE * 10;
  drum.pitch = randomf(0.0f, 0.025f);
