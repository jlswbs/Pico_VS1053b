// Wolfram cellular automata oscillator - temporal concept //
...
void loop() {
  ...
  if (counter >= 4) {
    counter = 0;
    int pattern = ((int)(y[0] > 0) << 2) | ((int)(y[1] > 0) << 1) | ((int)(y[2] > 0));
    float new_y = ((RULE >> pattern) & 1) ? 1.0f : -1.0f;
    y[2] = y[1];
    y[1] = y[0];
    y[0] = new_y;
  }
  ...
  acc = 0.995f * acc + 0.005f * y[0];

  int16_t l = (int16_t)(acc * 32767.0f);
  int16_t r = l;
...
void loop1() {
  ...
  freq = random(30, 220);
  RULE = random(0, 256);
  phase_inc = (freq * 0.25f) / (float)SAMPLE_RATE;
  delay(150);
