// VS1053 MIDI - Perlin noise music //

/*

  Based to Perlin noise model

  Pot1 = perlin frequency
  Pot2 = perlin x position
  Pot3 = tempo
...
void loop(){
  ...
  float freq = map(analogRead(A1), MINADC, MAXADC, 49, 999);
  freq /= 10000.0f;

  uint8_t decay = map(analogRead(A2), MINADC, MAXADC, 0, SIZE);

  uint8_t note = SIZE * perlin2d(decay, t, freq, ITER);

  uint8_t notes = map(note, 0, SIZE, 24, 96); // quantise note
  uint8_t prog = map(note, 0, SIZE, 0, 127);
  if (prog >= 18 && prog <= 23) prog = 0; // replace some instruments to grand piano
  uint8_t vol = map(note, 0, SIZE, 48, 116);
  
  note_on(rand() % 6, prog & 123, notes & 127, vol & 127);
  
  uint8_t drum = map(note/2, 0, SIZE, 35, 66); // note 27-87 drum kit
  uint8_t vold = map(note, 0, SIZE, 48, 116);
