// Curtis Roads - Pulsar synthesis + comb filter //
...
#define BPM 120
  ...
  float pulse_density = 15.0f;
  float pulse_len = 0.001f;
  float freq = 440.0f;
  float amp = 0.5f;
  float sigma = 0.5f;
  float comb_freq = 0.0f;
  float comb_feed = 0.95f;

  float pulse_phase = 0.0f;
  float current_time = 0.0f;
  float next_pulse_time = 0.0f;

float comb_filter(float x, float freq, float feedback, float fs) {

    const int MAX_DELAY = 512;
    static float buffer[MAX_DELAY];
    static int idx = 0;

    int delay = (int)(fs / freq);
    if (delay < 1) delay = 1;
    if (delay >= MAX_DELAY) delay = MAX_DELAY - 1;

    int readIdx = idx - delay;
    if (readIdx < 0) readIdx += MAX_DELAY;

    float y = buffer[readIdx];
    float out = x + y * feedback;
...
void loop() {
    ...
    float sample_f = comb_filter(pulsar_sample(dt), comb_freq, comb_feed, SAMPLE_RATE);
