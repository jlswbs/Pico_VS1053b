...
void setup() {
  ...
  dsp.begin(SAMPLE_RATE);
  synth.begin(SAMPLE_RATE, 1024);
  synth.noteOff();
  ...
  lp.begin(SAMPLE_RATE, 1);
  lp.setupFilter(FilterNode::LPF, 1500, 1.7);

  sat.begin(SAMPLE_RATE, 1);
  sat.setDrive(0.3f);
...
void loop() {
  ...
  if (ReadReg(SCI_AICTRL0) > 32){
    ...
    for (int i = 0; i < 32; i++) {
      ...
      int16_t sample =  16384 * dsp.process(sampleMono, 1);
