// VS1053 MIDI - Markov chain music //
...
#define BPM   120
  ...
  const char inputText[] = "HELLO hello 123456789";
  const int textLength = sizeof(inputText) - 1;
  const int maxChars = 128;
  int markovMatrix[maxChars][maxChars];
  int markovTotal[maxChars];
  char currentChar;

char nextChar(char c) {

  int idx = c;
  if (markovTotal[idx] == 0) {
    return inputText[random(textLength)];
  }
  int r = random(markovTotal[idx]);
  int sum = 0;
  for (int i = 0; i < maxChars; i++) {
    sum += markovMatrix[idx][i];
    if (r < sum) {
      return (char)i;
    }
  }
...
void setup(){
  ...
  currentChar = inputText[random(textLength)];
...
void loop(){
  ...
  uint8_t note = currentChar % 128;
  
  uint8_t notes = map(note, 0, 127, 12, 64); // quantise note C1-c4
  uint8_t vol = map(note, 0, 127, 32, 120);
  note_on(0, 0, notes & 127, vol & 127);
  note_on(1, 0, notes & 127, vol & 127);
  note_on(2, 2 + (note & 12), 6 + (notes/2), vol & 127);
  
  uint8_t drum = map(note, 0, 127, 66, 35); // note 27-87 drum kit
  uint8_t vold = map(note, 0, 127, 48, 108);

  if (drumon == true) note_on(9, 0, drum & 127, vold & 127);
  drumon = !drumon;
