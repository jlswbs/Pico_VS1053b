...
Compilation options: -lang c -ct 1 -es 1 -mcd 16 -mdd 32768 -mdy 33 -single -ftz 0
------------------------------------------------------------ */
...
static void fillmydspSIG0(mydspSIG0* dsp, int count, float* table) {
	/* C99 loop */
	{
		...
		for (i1 = 0; i1 < count; i1 = i1 + 1) {
			dsp->iVec0[0] = 1;
			dsp->iRec0[0] = (dsp->iVec0[1] + dsp->iRec0[1]) % 32768;
...
void instanceConstantsmydsp(mydsp* dsp, int sample_rate) {
	...
	dsp->fConst2 = 2.2e+02f / dsp->fConst0;
}
...
void computemydsp(mydsp* dsp, int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
	...
	/* C99 loop */
	{
		...
		for (i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp11 = fSlow10 * dsp->fRec4[0] * ftbl0mydspSIG0[max(0, min((int)(32768.0f * dsp->fRec1[0]), 32767))];
