...
const static int fand[180] PROGMEM = {
...
const static int harmo[180] PROGMEM = {
...
int Synth::calculate() {
  ...
  if (noise) wave = pgm_read_word(&(fand[rand()%180])) * 32 + pgm_read_word(&(harmo[i])) * (32 - harm);
  else wave = pgm_read_word(&(fand[i])) * 32 + pgm_read_word(&(harmo[i])) * (32 - harm);

  return vol * ((int)(wave * k) >> 7);
