// VS1053 MIDI - Multi logistic equation music //

/*

  Pot1 = chaos constant
  Pot2 = instruments offset
  Pot3 = tempo
...
void loop(){
  ...
  uint16_t valr = map(analogRead(A1), MINADC, MAXADC, 36500, 39800);
  ...
  for (int i = 0; i < POLY; i++) {
    ...
    nx[i] = x[i];
    x[i] = r[i] * nx[i] * (1.0f - nx[i]);

    outx[i] = constrain(127.0f * x[i], 0, 127);
  
  }

  uint16_t offset = map(analogRead(A2), MINADC, MAXADC, 63, 127);

  for (int i = 0; i < POLY; i++) {
  
    note[i] = map(outx[i], 0, 127, 12, 72); // quantise note C1-c4
    prog[i] = offset - outx[i];
    if (prog[i] >= 18 && prog[i] <= 23) prog[i] = 0; // replace some instruments to grand piano
    vel[i] = map(outx[rand() % POLY], 0, 127, 32, 96);
    note_on(i, prog[i] & 123, note[i] & 127, vel[i] & 127);

  }

  uint16_t tempo = map(analogRead(A3), MINADC, MAXADC, MINTEMPO, MAXTEMPO);
  uint16_t delay_ms = 60000 / tempo;

  uint8_t velb = map(outx[rand() % POLY], 0, 127, 48, 96);
  uint8_t drum = map(outx[rand() % POLY], 0, 127, 35, 66); // note 27-87 drum kit
  
  note_on(9, 0, drum & 127, velb & 127);
  if (drumon == true) note_on(9, 0, 36, 110 & 127); // bass drum
  drumon = !drumon;

  delay(delay_ms / 4);

  uint8_t rel_off = rand() % 8;
