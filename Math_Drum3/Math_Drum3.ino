//  VS1053b PCM mode stereo 8bit 22050 Hz - mathematical resonant membrane drum 3 //
...
float Synth::calculate() {
  ...
  float nois = randomf(0.0f, noise);

  if (swap == true) osc = p * v;
  else osc = v + nois;
...
void loop1(){
  ...
  drum.delta = randomf(0.09f, 0.99f);
  drum.noise = randomf(0.0f, 0.49f);
