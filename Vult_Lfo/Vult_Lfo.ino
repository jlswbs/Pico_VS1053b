...
#define SAMPLE_RATE       22050
  ...
  uint16_t tune = 1000; // lfo tune  
  uint16_t shape = 0;   // lfo shape saw-tri-sqr
  uint16_t rst = 0;     // lfo reset
...
void loop() {
  ...
  if (ReadReg(SCI_AICTRL0) > 32){
    ...
    for (int i = 0; i < 32; i++) {
      ...
      int16_t sample = Phasedist_process(phasedist, cv , Lfo_process(lfo, tune, shape, rst)) >> 2;
