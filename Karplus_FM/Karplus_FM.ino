...
void init_karplus_fm(float f1, float f2, float mod_idx) {
    freq1 = f1;
    freq2 = f2;
    mod_index = mod_idx;

    delay_len = int(SAMPLE_RATE / freq1);
...
float karplus_fm_sample() {

    float phase_inc = freq1 / SAMPLE_RATE * (1.0f + mod_index * sin(2.0f * 3.14159265f * freq2 * n_samples / SAMPLE_RATE));
    ...
    float new_sample = cutoff * (cur + prev_sample) / 2.0f;
    ks_buf[i0] = new_sample * feedback;
    prev_sample = new_sample;

    ks_index_f += phase_inc * delay_len;
...
void loop() {
    ...
    uint8_t sample = 128 + 127.0f * karplus_fm_sample();
...
void loop1() {
    ...
    freq1 = random(100, 880);
    freq2 = random(100, 3000);

    init_karplus_fm(freq1, freq2, mod_index);
