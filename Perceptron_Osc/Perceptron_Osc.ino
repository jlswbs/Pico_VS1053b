// Multi perceptron harmonic modal oscillator //
...
void loop() {
  ...
  for(int p=0; p<perceptrons; p++) {
    ...
    float x = TANH(w0*x_prev + w1*x_prev2 + b - x);
  ...
  for(int i=0; i<perceptrons; i++) {
    ...
    sum += W_out[i] * outputs[i];

  }

  float sample = TANH(sum + B_out);

  int16_t l = (int16_t)(32767.0f * sample);
  int16_t r = l;
