// Vult DSP Adsr envelope example //
...
Sine_process_type sine;
Adsr_process_type env;
...
uint16_t rst = 0;
uint16_t gate = 0;
uint16_t attack = 2000;
uint16_t decay = 5000;
uint16_t sustain = 15000;
uint16_t release = 10000;
...
void setup() {
  ...
  Sine_process_init(sine);
  Adsr_process_init(env);
...
void loop() {
  ...
  if (ReadReg(SCI_AICTRL0) > 32){
    ...
    for (int i = 0; i < 32; i++) {
      ...
      int16_t sample = (Adsr_process(env, gate, attack, decay, sustain, release) * Sine_process(sine, cv, rst)) >> 11;
