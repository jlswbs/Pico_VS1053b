// VS1053 MIDI - Bytebeat generator music //
...
void pitch(byte chan, unsigned int pitch){

  unsigned int change = pitch;
  ...
  PicoSPI0.transfer(0x00);  
  PicoSPI0.transfer(low);
  PicoSPI0.transfer(0x00);
  PicoSPI0.transfer(high);
  PicoSPI0.transfer(0x00);
...
void setup(){
  ...
  WriteReg16(SCI_CLOCKF, 0x8BE8); // Set clock multiplier to 3.5x
  WriteReg16(SCI_AUDATA, 44100);  // Set sample rate
  WriteReg(SCI_VOL, 50, 50);      // Set initial volume (20 = -10dB)
...
void loop(){
  ...
  uint16_t algos = map(analogRead(A1), MINADC, MAXADC, 0, 10);  

  uint16_t srate = map(analogRead(A2), MINADC, MAXADC, 4410, 44100);
  WriteReg16(SCI_AUDATA, srate);  // set samplerate

  uint16_t del = map(analogRead(A3), MINADC, MAXADC, 239, 99);

  switch(algos){

    case  0: out = ( t*(t>>9|t>>10)%(t|t>>9) ) % 16384; break;
    case  1: out = ( t*(t>>9&t>>13)%(t|t>>9) ) % 16384; break;
    case  2: out = ( t&(t>>9|t>>13)%(t|t>>9) ) % 16384; break;
    case  3: out = ( t|(t>>6^t>>13)%(t|t>>9) ) % 16384; break;
    case  4: out = ( t^(t>>9^t>>13)+(t|t>>9) ) % 16384; break;
    case  5: out = ( t^(t>>9^t>>13)-(t|t>>9) ) % 16384; break;
    case  6: out = ( (t>>9^t>>13)-(t|t>>9) ) % 16384; break;
    case  7: out = ( (t<<2^t>>10)-(t|t>>9) ) % 16384; break;
    case  8: out = ( (t<<0^t>>10)-(t|t>>9) ) % 16384; break;
    case  9: out = ( ((t<<t)^t>>10)+(t|t>>9) ) % 16384; break;
    case 10: out = ( ((t<<t)^t>>10)+(t^t>>9) ) % 16384; break;
  ...
  if (cnt == del) {
    ...
    cnt = 0; 
    note_on(0, out%128, 64+16, 48+rand()%36);
    note_on(1, 127-out%128, 64, 48+rand()%36);  
    
    if(dr) {
      
      note_on(9, 0, 35, 64);
      note_on(9, 0, 35+out%36, 24+rand()%24);
