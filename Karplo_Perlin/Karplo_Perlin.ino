//  VS1053b PCM mode stereo 8bit 22050 Hz - Karplus membrane with perlin noise exciter //
...
public:
  ...
  void trigger(int val, float val2);
	float calculate();

};

void Synth::trigger(int lenght, float freq) {

  for (int i = 0; i < SIZE; i++) delaymem[i] = 0;
	for (int i = 0; i < lenght; i++) delaymem[i] = perlin2d(i, lenght, freq, ITER);
...
float Synth::calculate() {
	...
	uint16_t bound;
  if (div) bound = 0.75f * SIZE;
  else bound = SIZE;

	delaymem[locat++] = clp - out;
  if (locat >= bound) locat = 0;
  curr = delaymem[locat];
  out = 0.5f * accum;
  accum = accum - (lpf * (accum - (last + curr))); 
  last = curr;
...
void loop(){
  ...
  uint8_t l = 128.0f + 127.0f * fabs(osc.calculate());
  uint8_t r = -l;
...
void loop1(){
  ...
  int bpm = map(analogRead(A3), MINADC, MAXADC, MINTEMPO, MAXTEMPO);
  int tempo = 60000 / bpm;

  int decay = map(analogRead(A1), MINADC, MAXADC, 8, SIZE);
  float freq = map(analogRead(A2), MINADC, MAXADC, 49, 999);
  freq /= 10000.0f;

  osc.trigger(decay, freq);
  osc.vol = 0.99f;
  osc.clp = 2.0f * freq;
  osc.lpf = 0.3f;
  osc.div = false;
