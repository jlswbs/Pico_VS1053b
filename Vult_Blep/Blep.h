...
static_inline int Blep_minblep_samples(){
   return 678;
};

static_inline fix16_t Blep_minblep(int ch, int i){
   if(ch == 0){
      return fix_wrap_array(Blep_minblep_chan_0)[(i % 678)];
   }
   return 0x0 /* 0.000000 */;
}
...
typedef Blep__ctx_type_2 Blep_minblep_play_type;

void Blep__ctx_type_2_init(Blep__ctx_type_2 &_output_);

static_inline void Blep_minblep_play_init(Blep__ctx_type_2 &_output_){
   Blep__ctx_type_2_init(_output_);
   return ;
}

fix16_t Blep_minblep_play(Blep__ctx_type_2 &_ctx, uint8_t trigger);

typedef struct Blep__ctx_type_3 {
   fix16_t pre_phase;
   int active_blep;
   fix16_t acc;
   Blep__ctx_type_2 _inst757;
   Blep__ctx_type_2 _inst657;
   Blep__ctx_type_2 _inst557;
   Blep__ctx_type_2 _inst457;
   Util__ctx_type_0 _inst251;
   Phase__ctx_type_0 _inst1cd;
} Blep__ctx_type_3;

typedef Blep__ctx_type_3 Blep_minblep_step_type;

void Blep__ctx_type_3_init(Blep__ctx_type_3 &_output_);

static_inline void Blep_minblep_step_init(Blep__ctx_type_3 &_output_){
   Blep__ctx_type_3_init(_output_);
   return ;
}

fix16_t Blep_minblep_step(Blep__ctx_type_3 &_ctx, fix16_t cv, uint8_t reset);

typedef struct Blep__ctx_type_4 {
   Blep__ctx_type_3 o;
} Blep__ctx_type_4;

typedef Blep__ctx_type_4 Blep_process_type;

static_inline void Blep__ctx_type_4_init(Blep__ctx_type_4 &_output_){
   Blep__ctx_type_4 _ctx;
   Blep__ctx_type_3_init(_ctx.o);
   _output_ = _ctx;
   return ;
}

static_inline void Blep_process_init(Blep__ctx_type_4 &_output_){
   Blep__ctx_type_4_init(_output_);
   return ;
}

fix16_t Blep_process(Blep__ctx_type_4 &_ctx, fix16_t cv, fix16_t reset);
...
public:
  Blep(void)
  {
    Blep_process_init(data);
  }

  int16_t output(int cv, int rst) { return Blep_process(data, cv, rst) >> 2; }

private:
  Blep_process_type data;
