
/* Code automatically generated by Vult https://github.com/modlfo/vult */
#include "Blep.h"

void Util__ctx_type_3_init(Util__ctx_type_3 &_output_){
   Util__ctx_type_3 _ctx;
   _ctx.y1 = 0x0 /* 0.000000 */;
   _ctx.x1 = 0x0 /* 0.000000 */;
   _output_ = _ctx;
   return ;
}

fix16_t Util_dcblock(Util__ctx_type_3 &_ctx, fix16_t x0){
   fix16_t y0;
   y0 = (x0 + (- _ctx.x1) + fix_mul(0xfeb8 /* 0.995000 */,_ctx.y1));
   _ctx.x1 = x0;
   _ctx.y1 = y0;
   return y0;
}

void Phase__ctx_type_0_init(Phase__ctx_type_0 &_output_){
   Phase__ctx_type_0 _ctx;
   _ctx.rate = 0x0 /* 0.000000 */;
   _ctx.phase = 0x0 /* 0.000000 */;
   Util__ctx_type_0_init(_ctx._inst351);
   Util__ctx_type_1_init(_ctx._inst13b);
   Phase_default(_ctx);
   _output_ = _ctx;
   return ;
}

fix16_t Phase_process(Phase__ctx_type_0 &_ctx, fix16_t cv, fix16_t reset){
   if(Util_change(_ctx._inst13b,cv)){
      _ctx.rate = Util_cvToRate_1024(cv);
   }
   if(Util_edge(_ctx._inst351,(reset > 0x8000 /* 0.500000 */))){
      _ctx.phase = 0x0 /* 0.000000 */;
   }
   else
   {
      _ctx.phase = (_ctx.phase + _ctx.rate);
   }
   if(_ctx.phase > 0x4000000 /* 1024.000000 */){
      _ctx.phase = (-0x4000000 /* -1024.000000 */ + _ctx.phase);
   }
   return (_ctx.phase >> 10);
}

void Blep__ctx_type_2_init(Blep__ctx_type_2 &_output_){
   Blep__ctx_type_2 _ctx;
   _ctx.playing = false;
   _ctx.index = 0;
   Util__ctx_type_0_init(_ctx._inst151);
   _output_ = _ctx;
   return ;
}

fix16_t Blep_minblep_play(Blep__ctx_type_2 &_ctx, uint8_t trigger){
   if(Util_edge(_ctx._inst151,trigger)){
      _ctx.index = 0;
      _ctx.playing = true;
   }
   fix16_t out;
   out = 0x0 /* 0.000000 */;
   if(_ctx.playing){
      if(_ctx.index >= Blep_minblep_samples()){
         _ctx.playing = false;
         _ctx.index = 0;
      }
      else
      {
         _ctx.index = (16 + _ctx.index);
      }
      if(_ctx.playing){
         out = Blep_minblep(0,_ctx.index);
      }
      else
      {
         out = 0x0 /* 0.000000 */;
      }
   }
   return out;
}

void Blep__ctx_type_3_init(Blep__ctx_type_3 &_output_){
   Blep__ctx_type_3 _ctx;
   _ctx.pre_phase = 0x0 /* 0.000000 */;
   _ctx.active_blep = 0;
   _ctx.acc = 0x0 /* 0.000000 */;
   Blep__ctx_type_2_init(_ctx._inst757);
   Blep__ctx_type_2_init(_ctx._inst657);
   Blep__ctx_type_2_init(_ctx._inst557);
   Blep__ctx_type_2_init(_ctx._inst457);
   Util__ctx_type_0_init(_ctx._inst251);
   Phase__ctx_type_0_init(_ctx._inst1cd);
   _output_ = _ctx;
   return ;
}

fix16_t Blep_minblep_step(Blep__ctx_type_3 &_ctx, fix16_t cv, uint8_t reset){
   fix16_t phase;
   phase = Phase_process(_ctx._inst1cd,cv,0x0 /* 0.000000 */);
   uint8_t wrap;
   wrap = ((_ctx.pre_phase + (- phase)) > 0x8000 /* 0.500000 */);
   _ctx.pre_phase = phase;
   if(wrap || Util_edge(_ctx._inst251,reset)){
      _ctx.active_blep = ((1 + _ctx.active_blep) % 4);
   }
   fix16_t o0;
   o0 = Blep_minblep_play(_ctx._inst457,(_ctx.active_blep == 0));
   fix16_t o1;
   o1 = Blep_minblep_play(_ctx._inst557,(_ctx.active_blep == 1));
   fix16_t o2;
   o2 = Blep_minblep_play(_ctx._inst657,(_ctx.active_blep == 2));
   fix16_t o3;
   o3 = Blep_minblep_play(_ctx._inst757,(_ctx.active_blep == 3));
   fix16_t pulses;
   pulses = (o0 + o2 + (- o1) + (- o3));
   _ctx.acc = (fix_mul(0xfef9 /* 0.996000 */,_ctx.acc) + fix_mul(0x140000 /* 20.000000 */,pulses));
   return _ctx.acc;
}

fix16_t Blep_process(Blep__ctx_type_4 &_ctx, fix16_t cv, fix16_t reset){
   uint8_t breset;
   breset = (reset > 0x8000 /* 0.500000 */);
   fix16_t out1;
   out1 = Blep_minblep_step(_ctx.o,cv,breset);
   fix16_t out2;
   out2 = Blep_minblep_step(_ctx.o,cv,breset);
   fix16_t out3;
   out3 = Blep_minblep_step(_ctx.o,cv,breset);
   fix16_t out4;
   out4 = Blep_minblep_step(_ctx.o,cv,breset);
   return ((out1 + out2 + out3 + out4) >> 2);
}


