// VLSI 1053 codec PCM mixer - Poly Karplus-strong with distortion and reverb example //

/*

  Based to Logistic equation

  Pot1 = chaos constant voice 1
  Pot2 = chaos constant voice 2
  Pot3 = tempo
...
void loop1(){
  ...
  r[0] = map(analogRead(A1), MINADC, MAXADC, 34999, 39999);
  r[0] /= 10000.0f;

  r[1] = map(analogRead(A2), MINADC, MAXADC, 34999, 39999);
  r[1] /= 10000.0f;
...
void loop(){
  ...
  if (ReadReg(SCI_AICTRL0) > 32){
    ...
    for (int k = 0; k < 32; k++) { 
      ...
      int sample = 32768 * dist_filter(outs, 0.15f);  
      ...
      output = sample + (reverbBuffer[reverbAddr]>>2);      
