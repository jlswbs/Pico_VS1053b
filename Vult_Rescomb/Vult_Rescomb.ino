// Vult DSP resonant comb filter example //
...
uint16_t res = 65535; // comb resonance
...
void loop() {
  ...
  if (ReadReg(SCI_AICTRL0) > 32){
    ...
    for (int i = 0; i < 32; i++) {    
      int16_t sample = Rescomb_process(comb, Saw_process(saw, cv)>>3, combcv, tones, res) >> 2;
