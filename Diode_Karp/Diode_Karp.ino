// VLSI 1053 codec PCM mixer - Karplus strong + diode clipping + echoverb //

/*

  Pot1 = threshold value
...
void loop(){
  ...
  int16_t sample = 32768.0f * echo_verb(diode_clip(out, thrs), 0.5f);
...
void loop1(){
  ...
  thrs = map(analogRead(A1), MINADC, MAXADC, 499, 9999);
  thrs /= 10000.0f;

  lowpass = map(analogRead(A2), MINADC, MAXADC, 499, 9999);
  lowpass /= 10000.0f;

  for (int i = 0; i < SIZE; i++) delaymem[i] = randomf(-1.0f, 1.0f);
  bound = map(chaos_rand(SIZE), 0, SIZE, 16, SIZE);
