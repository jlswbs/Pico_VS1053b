...
Swept_process_type swept;
Dline_process_type del;
...
void setup() {
  ...
  Noise_process_init(noise);  
  Swept_process_init(swept);    
  Dline_process_init(del);  
...
void loop() {
  ...
  if (ReadReg(SCI_AICTRL0) > 32){
    ...
    for (int i = 0; i < 32; i++) {
      ...
      int16_t sample = (Swept_process(swept, gate, 0, 48, sweptcv) * Noise_process(noise, 65535)) >> 8;
